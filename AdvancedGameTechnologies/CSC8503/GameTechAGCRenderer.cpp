...
GameTechAGCRenderer::GameTechAGCRenderer(GameWorld& world) : AGCRenderer(*Window::GetWindow()), gameWorld(world) {
	...
	std::cout << Window::GetWindow()->GetScreenSize().x << " " << Window::GetWindow()->GetScreenSize().y << std::endl;
...
Mesh* GameTechAGCRenderer::GenerateTerrainMesh(HeightMap* heightmap) {
	terrainMesh = new AGCMesh();
	...
	terrainMesh->UploadToGPU(this);
	//BindMesh(*terrainMesh);

	return terrainMesh;
}

Vector3 GameTechAGCRenderer::CalculateNormal(int x, int y, HeightMap* heightmap) {
...
Texture* GameTechAGCRenderer::LoadTexture(const std::string& name) {
	/*std::cout << "Loading texture from: " << name << std::endl;
	if (!std::filesystem::exists(name)) {
		std::cerr << "Error: File does not exist: " << name << std::endl;
		return nullptr;
	}*/
	std::filesystem::path path(name + ".gnf");
	//path.replace_extension(".gnf");
	std::string realFile = Assets::TEXTUREDIR + path.string();
	//std::cout << realFile << std::endl;
...
void GameTechAGCRenderer::GPUSkinningPass() {
	if (frameJobs.empty()) {
		std::cout << "GPUSkinningPass: No frame jobs to process, exiting.\n";
		return;
	}

	std::cout << "GPUSkinningPass: Setting compute shader.\n";
	...
	std::cout << "GPUSkinningPass: Dispatch modifier set.\n";
	...
	for (auto& i : frameJobs) {
		...
		std::cout << "GPUSkinningPass: Attempting to retrieve mesh buffers.\n";
		if (!m->GetAGCBuffer(VertexAttribute::Positions, inputBuffers[0]) ||
			!m->GetAGCBuffer(VertexAttribute::Normals, inputBuffers[1]) || //for some reason this causes a crash, and then later on it says invalid format, so player had some sort of issue with its vertex attribute types
			...
			std::cout << "GPUSkinningPass: Missing one or more vertex buffers, skipping job.\n";
			continue;
		}
		std::cout << "GPUSkinningPass: Successfully retrieved mesh buffers.\n";

		char* offset = currentFrame->data.data;
		std::cout << "GPUSkinningPass: Offset for skeleton data set.\n";

		std::vector<Matrix4>& skeleton = i.object->GetSkeleton();
		std::cout << "GPUSkinningPass: Writing skeleton data, size: " << skeleton.size() << "\n";
		...
		std::cout << "GPUSkinningPass: BufferSpec initialized for skeleton data.\n";

		SceError error = sce::Agc::Core::initialize(&inputBuffers[5], &bufSpec);
		if (error != SCE_OK) {
			std::cout << "GPUSkinningPass: Error initializing skeleton buffer: " << error << "\n";
		}
		else {
			std::cout << "GPUSkinningPass: Skeleton buffer initialized successfully.\n";
		}

		inputBuffers[5].setFormat(sce::Agc::Core::Buffer::Format::k32_32_32_32Float);
		std::cout << "GPUSkinningPass: Skeleton buffer format set.\n";

		std::cout << "GPUSkinningPass: Setting buffers for compute shader.\n";
		...
		std::cout << "GPUSkinningPass: Dispatching compute shader with " << threadCount << " thread groups.\n";
		frameContext->m_dcb.dispatch(threadCount, 1, 1, modifier);
	}

	std::cout << "GPUSkinningPass: Clearing frame jobs.\n";
	frameJobs.clear();

	std::cout << "GPUSkinningPass: Resetting compute shader.\n";
...
void GameTechAGCRenderer::UpdateObjectList() {
		...
		[&](GameObject* o) {
			...
			if (o->IsActive()) {
				...
				if (g) {
					...
					std::cout << "Created Buffer b" << std::endl;
						if (!b) {
							std::cout << "No GPU Buffer in b" << std::endl;
							...
							std::cout << "Vertex Size: " << vertexSize << "Getting Vertex Count"  << std::endl;
							size_t vertexCount = m->GetVertexCount();
							std::cout << "Vertex Count: " << vertexCount << "Getting bufferSize" << std::endl;
							...
							std::cout << "Allocated Vertex Data" << vertexData << std::endl;

							sce::Agc::Core::BufferSpec bufSpec;
							std::cout << "Buffer Spec" << std::endl;
							bufSpec.initAsRegularBuffer(vertexData, vertexSize, vertexCount);
							std::cout << "Buffer Spec Initialized with:" << "Data: " << vertexData << "Size: " << vertexSize << "Count :" << vertexCount << std::endl;

							sce::Agc::Core::Buffer vBuffer;
							std::cout << "Buffer vBuffer" << std::endl;
							...
							std::cout << "Created AGCBuffer with b" << std::endl;
							b->SetAssetID(bufferID);
							std::cout << "Set Asset ID" << std::endl;
							g->SetGPUBuffer(b);
							std::cout << "Set GPU Buffer" << std::endl;

							bindlessBuffers[bufferID] = vBuffer;
							std::cout << "Bindless Buffers" << std::endl;
						}
						state.index[1] = b->GetAssetID();
						std::cout << "Got Asset ID" << std::endl;

						frameJobs.push_back({ g, b->GetAssetID() });
						std::cout << "Pushed Back frameJobs" << std::endl;
					}
					currentFrame->data.WriteData<ObjectState>(state);
					std::cout << "Wrote currentFrame Data" << std::endl;
					currentFrame->debugLinesOffset += sizeof(ObjectState);
					std::cout << "Incremented Debug Lines Offset" << std::endl;
					at++;
					std::cout << "Incremented at" << std::endl;
				}
			}
		}
	);
	...
	std::cout << "Buffer Spec Initialized with:" << "Data: " << dataPos << "Size: " << sizeof(ObjectState) << "Count :" << at << std::endl;
