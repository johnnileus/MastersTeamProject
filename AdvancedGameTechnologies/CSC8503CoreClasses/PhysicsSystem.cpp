...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			// 调用 ObjectIntersection 检测碰撞
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {

				ImpulseResolveCollision(*info.a, *info.b, info.point);
				info.framesLeft = numCollisionFrames; // 设置碰撞持续帧数
				allCollisions.insert(info); // 将碰撞信息存入集合
			}
		}
	}
}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b,CollisionDetection::ContactPoint& p) const
{
	...
	float cRestitution = 0.66f; // 硬编码的恢复系数（可以扩展为动态变量）
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		object->UpdateInertiaTensor(); // 更新张量以匹配方向

		Vector3 angAccel = object->GetInertiaTensor() * torque;

		angVel += angAccel * dt; // 整合角加速度！
